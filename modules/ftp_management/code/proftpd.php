...
if (!fs_director::CheckForEmptyValue(self::$create)) {
    ...
    $sql->execute();
    $sql = $ftp_db->prepare("INSERT INTO ftpuser (id, userid, passwd, homedir, shell, count, accessed, modified) VALUES (null, :username, :password, :homedir, '/sbin/nologin', 0, :datetime, :datetime);");
    $sql->bindParam(':username', $username);
    $sql->bindParam(':password', $password);
    $sql->bindParam(':homedir', $homedir);
    $sql->bindParam(':datetime', date('Y-m-d H:i:s'));
    $sql->execute();
}
